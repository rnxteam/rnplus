...
const fixedLogic = (context, em, isView) => {
  ...
  for (const key in bindEvents) {
    if (_isFunction(bindEvents[key])) {
      let bindEventsBind = bindEvents[key].bind(context);
      if(isView)
        em.on(key, ()=>{
          bindEventsBind();
          context.subComponentEventManagers.forEach(em=>{
            em.trigger(key);
          });
        });
      else
  ...
  class ExtComp extends React.Component {
    constructor(props, context) {
      ...
      if(isView) {  // pView初始化
        ...
        this.pViewEventRegister = (function(callback) {
          if(this.subComponentEventManagers.indexOf(callback) === -1)
            this.subComponentEventManagers.push(callback);
        }).bind(this);

        this.pViewEventUnRegister = (function(callback) {
          console.log('pViewEventUnRegister todo');
        }).bind(this);

        this.getChildContext = (function() {
          return {
            pViewEventRegister: this.pViewEventRegister,
            pViewEventUnRegister: this.pViewEventUnRegister
          }
        }).bind(this);

      }
      else {  // pComponent初始化
        if(context && context.pViewEventRegister) {
          context.pViewEventRegister(em);
        }
