...
const fixedLogic = (context, em, isView) => {
  ...
  ['actived', 'deactived', 'ready'].forEach(key => {
    ...
    const bindEventsBind = bindEvents && bindEvents[key] && bindEvents[key].bind(context);
    if (isView) {
      em.on(key, (params) => {
        if (bindEventNames.indexOf(key) > -1) { // 页面级事件
          if (bindEventsBind) {
            bindEventsBind(params);
          }
        }

        if (context.subComponentEventManagers[key]) {
          context.subComponentEventManagers[key].forEach((subEm) => {  // 组件级事件
            subEm.trigger(key, params);
          });
        }
      });
    } else {
...
function createComponent(isView) {
  class ExtComp extends React.Component {
    constructor(props, context) {
      ...
      const originUnmountFn = this.componentWillUnmount;
      ...
      // 组件卸载后释放组件中定义的 bindEvents
      this.componentWillUnmount = function () {
        this.safelyFlag = true;
        let rs = NULL;
        if (_isFunction(originUnmountFn)) {
          try {
            const bindEvents = this.bindEvents && this.bindEvents;
            this.setStateSafely = NULL;
            this.safelyFlag = NULL;
            delete this.setStateSafely;
            if (_isObject(bindEvents)) {
              this.bindEvents = NULL;
              delete this.bindEvents;
            }
            rs = originUnmountFn.apply(this, arguments);
          } catch (e) {
            console.log('[RNPlus Error]', e);
            throw e;
          }
      ...
      // 安全更改 state 接口
      this.setStateSafely = function (partialState, callback) {
        try {
          if (_isObject(partialState)) {
            if (!this.safelyFlag) {
              let safelyState = NULL;
              safelyState = Object.assign({}, this.state, partialState);
              this.setState({
                ...safelyState,
              }, _isFunction(callback));
            }
          }else {
            throw new Error('The partialState is a object、 callback is a function, please check type of param');
          }
        } catch (e) {
