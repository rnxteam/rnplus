...
let register = (RNPlusClass, RNPlusClassName) => {
    let finalClass = registerCallbacks.reduce((pre, cur) => {

        let isView = pre.type == 'View';
        let plugins = defaults.globalPlugins.concat(pre.plugins || []).filter(
            (key, index, source) => key && key[0] != '-' && source.indexOf(`-${key}`) == -1 && source.indexOf(key) == index
        )
 ...
 * @param  {Boolean} isInner 是否是内部调用
 * @return {[type]}          [description]
 */
function getCache(name, isInner) {
    if (!cache[name]) {
        init();
    }
...
function defineStateProperty(obj, attr, fn) {
...
defineStateProperty(RNPlus, 'View', () => getCache('View'));
defineStateProperty(RNPlus, 'Component', () => getCache('Component'));
defineStateProperty(global, 'QView', () => getCache('View'));
defineStateProperty(global, 'QComponent', () => getCache('Component'));
defineStateProperty(global, '__QView', () => getCache('View', true));
defineStateProperty(global, '__QComponent', () => getCache('Component', true));
...
// 暴露一下
global.React = React;
global.RNPlus = React.RNPlus = RNPlus;
